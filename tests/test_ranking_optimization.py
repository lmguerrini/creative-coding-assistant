...
from creative_coding_assistant.orchestration import (
    ...
    ranking_optimization_signal_by_id,
    ranking_optimization_signals_for_confidence,
    ranking_optimization_signals_for_status,
    route_request,
)
from creative_coding_assistant.orchestration.routing import RouteName

REQUIRED_EXECUTION_MODES = ("manual_mode", "assisted_mode", "auto_mode")
EXPECTED_ROADMAP_ITEMS = ("Ranking Optimization",)
REQUIRED_SIGNAL_FIELDS = {
    ...
    "execution_mode_id",
    "optimization_axis",
    "retrieval_evolution_signal_ids",
    "retrieval_evolution_signal_count",
    "source_count",
    "domain_count",
    "optimization_signal_summary",
    "ranking_signal_score",
    "ranking_policy_score",
    "governance_alignment_score",
    "mutation_risk_score",
    "governance_weight",
    "optimization_score",
    "hitl_required_before_ranking_optimization",
    "context_tags",
    "explainability_notes",
    "advisory_actions",
    "evidence",
    "blocked_runtime_behaviors",
    "ranking_optimization_capability_implemented",
    "ranking_optimization_metadata_implemented",
    "retrieval_evolution_metadata_used",
    "ranking_optimization_execution_implemented",
    "ranking_profile_mutation_implemented",
...
class RankingOptimizationTests(unittest.TestCase):
    def test_plan_builds_ranking_optimization_metadata(self) -> None:
        plan = build_ranking_optimization(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "ranking_optimization")
        self.assertEqual(plan.serialization_version, "ranking_optimization_plan.v1")
        self.assertEqual(plan.route_name, RouteName.GENERATE)
        self.assertEqual(plan.task_type, "creative_coding")
        self.assertEqual(plan.retrieval_evolution_role, "retrieval_evolution")
        self.assertEqual(
            plan.retrieval_evolution_serialization_version,
            "retrieval_evolution_plan.v1",
        )
        self.assertEqual(len(plan.retrieval_evolution_signal_ids), 5)
        self.assertEqual(plan.retrieval_evolution_signal_count, 5)
        ...
        self.assertFalse(plan.planned_ranking_optimization_ids)
        ...
        self.assertEqual(plan.overall_optimization_posture, "guarded")
        self.assertIn("does not execute ranking optimization", plan.authority_boundary)
        self.assertIn("write KB storage", plan.authority_boundary)
        self.assertTrue(plan.ranking_optimization_capability_implemented)
        self.assertTrue(plan.ranking_optimization_metadata_implemented)
        self.assertTrue(plan.roadmap_item_covered)
        self.assertTrue(plan.retrieval_evolution_metadata_used)
        self.assertFalse(plan.ranking_optimization_execution_implemented)
        self.assertFalse(plan.ranking_profile_mutation_implemented)
    ...
    def test_signals_score_ranking_optimization_without_execution(self) -> None:
        plan = build_ranking_optimization(route="generate")
        retrieval_signal_ids = set(plan.retrieval_evolution_signal_ids)
        ...
        for signal in plan.signals:
            ...
            self.assertEqual(
                ...
                "ranking_optimization_entry.v1",
            )
            self.assertEqual(signal.route_name, RouteName.GENERATE)
            self.assertEqual(
                signal.signal_id,
                f"ranking_optimization::{signal.signal_kind}",
            )
            self.assertEqual(
                signal.retrieval_evolution_signal_count,
                len(signal.retrieval_evolution_signal_ids),
            )
            self.assertTrue(
                set(signal.retrieval_evolution_signal_ids).issubset(
            ...
            self.assertEqual(
                ...
                min(
                    ...
                    max(
                        0,
                        signal.ranking_signal_score * 3
                        + signal.ranking_policy_score * 3
            ...
            self.assertIn("ranking_optimization", signal.context_tags)
            self.assertIn(
                "ranking_optimization_execution",
                signal.blocked_runtime_behaviors,
            )
            self.assertIn("retrieval_reranking", signal.blocked_runtime_behaviors)
            self.assertIn("kb_storage_write", signal.blocked_runtime_behaviors)
            self.assertTrue(signal.hitl_required_before_ranking_optimization)
            self.assertTrue(signal.ranking_optimization_capability_implemented)
            self.assertTrue(signal.ranking_optimization_metadata_implemented)
            self.assertTrue(signal.retrieval_evolution_metadata_used)
            self.assertFalse(signal.ranking_optimization_execution_implemented)
            self.assertFalse(signal.ranking_profile_mutation_implemented)
        ...
        inventory = ranking_optimization_signal_by_id(
        ...
        self.assertEqual(
            len(ranking_optimization_signals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(ranking_optimization_signals_for_confidence("high", plan)),
            2,
        )

    def test_plan_rejects_mismatched_ranking_optimization_metadata(self) -> None:
        plan = build_ranking_optimization()
        payload = plan.model_dump(mode="json")
        payload["signal_ids"] = ("missing",) + tuple(payload["signal_ids"][1:])

        with self.assertRaisesRegex(ValueError, "signal_ids must match"):
            RankingOptimizationPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["overall_optimization_score"] -= 1

        with self.assertRaisesRegex(
            ValueError,
            "overall_optimization_score must match",
        ):
            RankingOptimizationPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["planned_ranking_optimization_ids"] = (plan.signal_ids[0],)

        with self.assertRaisesRegex(
            ValueError,
            "planned_ranking_optimization_ids must remain empty",
        ):
            RankingOptimizationPlan(**payload)

    def test_ranking_optimization_composes_task_5_metadata(self) -> None:
        retrieval_plan = build_retrieval_evolution(route=RouteName.REVIEW)
        plan = build_ranking_optimization(
            route=RouteName.REVIEW,
            retrieval_evolution=retrieval_plan,
        )

        self.assertEqual(plan.route_name, RouteName.REVIEW)
        self.assertEqual(
            plan.retrieval_evolution_signal_ids,
            retrieval_plan.signal_ids,
        )
        self.assertEqual(plan.source_count, retrieval_plan.source_count)
        self.assertEqual(plan.domain_count, retrieval_plan.domain_count)
        for signal in plan.signals:
            self.assertTrue(
                set(signal.retrieval_evolution_signal_ids).issubset(
                    set(retrieval_plan.signal_ids)
                )
            )
            self.assertFalse(signal.ranking_optimization_execution_implemented)
            self.assertFalse(signal.retrieval_reranking_implemented)

    def test_ranking_optimization_preserves_routing_and_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review ranking optimization posture for p5.js examples.",
    ...
    def test_ranking_optimization_omits_runtime_mutation_calls(self) -> None:
        plan = build_ranking_optimization(route=RouteName.GENERATE)
        combined_text = " ".join(
            (
                ...
                *(
                    ...
                    for field in (
                        ...
                        signal.status,
                        signal.confidence,
                        signal.optimization_axis,
                        *signal.retrieval_evolution_signal_ids,
                        signal.optimization_signal_summary,
                        *signal.context_tags,
