...
from creative_coding_assistant.orchestration import (
    ...
    cost_benefit_analysis_proposal_by_id,
    cost_benefit_analysis_proposals_for_confidence,
    cost_benefit_analysis_proposals_for_status,
    route_request,
)
...
EXPECTED_ROADMAP_ITEMS = ("Cost / Benefit Analysis",)


class CostBenefitAnalysisTests(unittest.TestCase):
    def test_plan_builds_cost_benefit_metadata(self) -> None:
        plan = build_cost_benefit_analysis(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "cost_benefit_analysis")
        self.assertEqual(
    ...
    def test_proposals_score_cost_benefit_without_budget_mutation(
        self,
    ) -> None:
        plan = build_cost_benefit_analysis(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "cost_benefit_analysis_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "cost_benefit_analysis")
            self.assertEqual(proposal.roadmap_item, "Cost / Benefit Analysis")
            self.assertIn("cost_trends", proposal.downstream_systems)
            self.assertIn("quality_cost_optimizer", proposal.downstream_systems)
            self.assertEqual(
            ...
            self.assertIn(f"cost_score:{proposal.cost_score}", proposal.evidence)
            self.assertIn(
        ...
        signal = cost_benefit_analysis_proposal_by_id(
            "cost_benefit_analysis::cost_benefit_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_cost_benefit_metadata(self) -> None:
        plan = build_cost_benefit_analysis()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            CostBenefitAnalysisPlan(**payload)
    ...
    def test_cost_benefit_analysis_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review cost benefit proposals.",
