...
from creative_coding_assistant.orchestration import (
    ...
    memory_evolution_policies_proposal_by_id,
...
EXPECTED_ROADMAP_ITEMS = ("Memory Evolution Policies",)


class MemoryEvolutionPoliciesTests(unittest.TestCase):
    def test_plan_builds_memory_policy_metadata(self) -> None:
        plan = build_memory_evolution_policies(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "memory_evolution_policies")
        self.assertEqual(
    ...
    def test_proposals_score_memory_policies_without_memory_mutation(self) -> None:
        plan = build_memory_evolution_policies(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "memory_evolution_policies_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "memory_evolution_policies")
            self.assertEqual(proposal.roadmap_item, "Memory Evolution Policies")
            self.assertIn("creative_memory_core_surface", proposal.downstream_systems)
            self.assertIn("memory_integration_boundaries", proposal.downstream_systems)
            self.assertEqual(
            ...
            self.assertIn("rollback_feasibility", proposal.evolution_report_sections)
        ...
        signal = memory_evolution_policies_proposal_by_id(
            "memory_evolution_policies::memory_policy_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_memory_policy_metadata(self) -> None:
        plan = build_memory_evolution_policies()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            MemoryEvolutionPoliciesPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["mutated_memory_ids"] = (plan.proposal_ids[0],)
    ...
    def test_memory_policies_do_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review memory evolution policy proposals.",
