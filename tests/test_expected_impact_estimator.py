...
from creative_coding_assistant.orchestration import (
    ...
    expected_impact_estimator_proposal_by_id,
    expected_impact_estimator_proposals_for_confidence,
    expected_impact_estimator_proposals_for_status,
    route_request,
)
...
EXPECTED_ROADMAP_ITEMS = ("Expected Impact Estimator",)


class ExpectedImpactEstimatorTests(unittest.TestCase):
    def test_plan_builds_expected_impact_metadata(self) -> None:
        plan = build_expected_impact_estimator(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "expected_impact_estimator")
        self.assertEqual(
    ...
    def test_proposals_score_expected_impact_without_impact_mutation(
        self,
    ) -> None:
        plan = build_expected_impact_estimator(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "expected_impact_estimator_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "expected_impact_estimator")
            self.assertEqual(proposal.roadmap_item, "Expected Impact Estimator")
            self.assertIn("quality_prediction_engine", proposal.downstream_systems)
            self.assertIn("improvement_ranking_engine", proposal.downstream_systems)
            self.assertEqual(
            ...
            self.assertIn(f"impact_score:{proposal.impact_score}", proposal.evidence)
            self.assertIn(
        ...
        signal = expected_impact_estimator_proposal_by_id(
            "expected_impact_estimator::impact_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_expected_impact_metadata(self) -> None:
        plan = build_expected_impact_estimator()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            ExpectedImpactEstimatorPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["mutated_output_ids"] = (plan.proposal_ids[0],)
    ...
    def test_expected_impact_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review expected impact proposals.",
