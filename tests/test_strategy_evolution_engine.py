...
from creative_coding_assistant.orchestration import (
    ...
    route_request,
    strategy_evolution_engine_proposal_by_id,
    strategy_evolution_engine_proposals_for_confidence,
    strategy_evolution_engine_proposals_for_status,
)
...
EXPECTED_ROADMAP_ITEMS = ("Strategy Evolution Engine",)


class StrategyEvolutionEngineTests(unittest.TestCase):
    def test_plan_builds_strategy_evolution_metadata(self) -> None:
        plan = build_strategy_evolution_engine(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "strategy_evolution_engine")
        self.assertEqual(
    ...
    def test_proposals_score_strategy_evolution_without_strategy_mutation(
        self,
    ) -> None:
        plan = build_strategy_evolution_engine(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "strategy_evolution_engine_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "strategy_evolution_engine")
            self.assertEqual(proposal.roadmap_item, "Strategy Evolution Engine")
            self.assertIn("execution_strategy_selection", proposal.downstream_systems)
            self.assertIn("strategy_learning", proposal.downstream_systems)
            self.assertEqual(
        ...
        signal = strategy_evolution_engine_proposal_by_id(
            "strategy_evolution_engine::strategy_signal_alignment",
            plan,
        )
        ...
        self.assertEqual(
            len(strategy_evolution_engine_proposals_for_confidence("guarded", plan)),
            2,
        )

    def test_plan_rejects_mismatched_strategy_metadata(self) -> None:
        plan = build_strategy_evolution_engine()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            StrategyEvolutionEnginePlan(**payload)
    ...
    def test_strategy_evolution_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review strategy evolution proposals.",
