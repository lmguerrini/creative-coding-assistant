...
from creative_coding_assistant.orchestration import (
    ...
    route_request,
    taste_evolution_engine_proposal_by_id,
    taste_evolution_engine_proposals_for_confidence,
    taste_evolution_engine_proposals_for_status,
)
...
EXPECTED_ROADMAP_ITEMS = ("Taste Evolution Engine",)


class TasteEvolutionEngineTests(unittest.TestCase):
    def test_plan_builds_taste_evolution_metadata(self) -> None:
        plan = build_taste_evolution_engine(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "taste_evolution_engine")
        self.assertEqual(
    ...
    def test_proposals_score_taste_evolution_without_taste_mutation(
        self,
    ) -> None:
        plan = build_taste_evolution_engine(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "taste_evolution_engine_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "taste_evolution_engine")
            self.assertEqual(proposal.roadmap_item, "Taste Evolution Engine")
            self.assertIn(
                "creative_memory_secondary_surface",
                proposal.downstream_systems,
            )
            self.assertIn("long_term_creative_memory", proposal.downstream_systems)
            self.assertEqual(
        ...
        signal = taste_evolution_engine_proposal_by_id(
            "taste_evolution_engine::taste_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_taste_evolution_metadata(self) -> None:
        plan = build_taste_evolution_engine()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            TasteEvolutionEnginePlan(**payload)
    ...
    def test_taste_evolution_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review taste evolution proposals.",
