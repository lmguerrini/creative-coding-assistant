...
REQUIRED_ENTRY_FIELDS = {
    ...
    "governance_weight",
    "core_surface_score",
    "hitl_required_before_core_surface_activation",
    "context_tags",
...
class KnowledgeEvolutionCoreSurfaceTests(unittest.TestCase):
    def test_plan_builds_core_surface_metadata(self) -> None:
        plan = build_knowledge_evolution_core_surface(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "knowledge_evolution_core_surface")
        self.assertEqual(
        ...
        self.assertFalse(plan.executed_update_ids)
    ...
    def test_entries_score_core_surface_without_activation(self) -> None:
        plan = build_knowledge_evolution_core_surface(route="generate")
        ...
        for entry in plan.entries:
            ...
            self.assertEqual(
                ...
                f"knowledge_evolution_core::{entry.surface_kind}",
            )
            ...
            self.assertIn("knowledge_evolution_core_surface", entry.context_tags)
        ...
        update = knowledge_evolution_core_surface_entry_by_id(
            "knowledge_evolution_core::knowledge_update_surface",
            plan,
        )
        self.assertIsNotNone(update)
        assert update is not None
        self.assertEqual(update.status, "guarded")
        self.assertEqual(update.confidence, "guarded")
        self.assertEqual(
            len(knowledge_evolution_core_surface_entries_for_status("guarded", plan)),
            2,
        )
        self.assertEqual(
            len(knowledge_evolution_core_surface_entries_for_confidence("high", plan)),
            1,
        )

    def test_plan_rejects_mismatched_core_surface_metadata(self) -> None:
        plan = build_knowledge_evolution_core_surface()
        payload = plan.model_dump(mode="json")
        payload["entry_ids"] = ("missing",) + tuple(payload["entry_ids"][1:])

        with self.assertRaisesRegex(ValueError, "entry_ids must match"):
            KnowledgeEvolutionCoreSurfacePlan(**payload)
    ...
    def test_core_surface_does_not_change_routing_or_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review the knowledge evolution core surface.",
