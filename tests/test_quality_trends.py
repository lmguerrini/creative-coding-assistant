...
from creative_coding_assistant.orchestration import (
    ...
    quality_trends_proposal_by_id,
    quality_trends_proposals_for_confidence,
    quality_trends_proposals_for_status,
    route_request,
)
...
EXPECTED_ROADMAP_ITEMS = ("Quality Trends",)


class QualityTrendsTests(unittest.TestCase):
    def test_plan_builds_quality_trend_proposal_metadata(self) -> None:
        plan = build_quality_trends(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "quality_trends")
        self.assertEqual(plan.serialization_version, "quality_trends_plan.v1")
    ...
    def test_proposals_score_quality_trends_without_output_mutation(self) -> None:
        plan = build_quality_trends(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "quality_trends_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "quality_trends")
            self.assertEqual(proposal.roadmap_item, "Quality Trends")
            self.assertIn("quality_dashboard", proposal.downstream_systems)
            self.assertEqual(
        ...
        drift = quality_trends_proposal_by_id(
            "quality_trends::quality_drift_signal",
            plan,
        )
        self.assertIsNotNone(drift)
        assert drift is not None
        self.assertEqual(drift.status, "guarded")
        self.assertEqual(
            len(quality_trends_proposals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(quality_trends_proposals_for_confidence("guarded", plan)),
            2,
        )

    def test_plan_rejects_mismatched_quality_trend_metadata(self) -> None:
        plan = build_quality_trends()
    ...
    def test_quality_trends_do_not_change_routing_or_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review quality trend evolution suggestions.",
