...
from creative_coding_assistant.orchestration import (
    ...
    knowledge_rollback_signal_by_id,
    knowledge_rollback_signals_for_confidence,
    knowledge_rollback_signals_for_status,
    route_request,
)
from creative_coding_assistant.orchestration.routing import RouteName

REQUIRED_EXECUTION_MODES = ("manual_mode", "assisted_mode", "auto_mode")
EXPECTED_ROADMAP_ITEMS = ("Knowledge Rollback",)
REQUIRED_SIGNAL_FIELDS = {
    ...
    "execution_mode_id",
    "rollback_axis",
    "knowledge_snapshot_signal_ids",
    "knowledge_snapshot_signal_count",
    "source_count",
    "domain_count",
    "rollback_signal_summary",
    "rollback_signal_score",
    "snapshot_alignment_score",
    "governance_alignment_score",
    "mutation_risk_score",
    "governance_weight",
    "rollback_score",
    "hitl_required_before_rollback",
    "context_tags",
    "explainability_notes",
    "advisory_actions",
    "evidence",
    "blocked_runtime_behaviors",
    "knowledge_rollback_capability_implemented",
    "knowledge_rollback_metadata_implemented",
    "knowledge_snapshot_metadata_used",
    ...
    "snapshot_selection_mutation_implemented",
...
class KnowledgeRollbackTests(unittest.TestCase):
    def test_plan_builds_knowledge_rollback_metadata(self) -> None:
        plan = build_knowledge_rollback(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "knowledge_rollback")
        self.assertEqual(plan.serialization_version, "knowledge_rollback_plan.v1")
        self.assertEqual(plan.route_name, RouteName.GENERATE)
        self.assertEqual(plan.task_type, "creative_coding")
        self.assertEqual(plan.knowledge_snapshot_role, "knowledge_snapshot_engine")
        self.assertEqual(
            plan.knowledge_snapshot_serialization_version,
            "knowledge_snapshot_plan.v1",
        )
        self.assertEqual(len(plan.knowledge_snapshot_signal_ids), 5)
        self.assertEqual(plan.knowledge_snapshot_signal_count, 5)
        ...
        self.assertFalse(plan.planned_rollback_ids)
        self.assertFalse(plan.applied_rollback_ids)
        self.assertFalse(plan.mutated_rollback_state_ids)
        self.assertFalse(plan.written_rollback_record_ids)
        self.assertFalse(plan.restored_snapshot_ids)
        self.assertFalse(plan.selected_snapshot_ids)
        self.assertFalse(plan.restored_kb_state_ids)
        self.assertFalse(plan.written_kb_record_ids)
        self.assertEqual(plan.overall_rollback_posture, "guarded")
        self.assertIn("does not execute rollback", plan.authority_boundary)
        self.assertIn("write KB storage", plan.authority_boundary)
        self.assertTrue(plan.knowledge_rollback_capability_implemented)
        self.assertTrue(plan.knowledge_rollback_metadata_implemented)
        self.assertTrue(plan.roadmap_item_covered)
        self.assertTrue(plan.knowledge_snapshot_metadata_used)
        ...
        self.assertFalse(plan.rollback_state_mutation_implemented)
        self.assertFalse(plan.rollback_record_write_implemented)
        self.assertFalse(plan.snapshot_restore_execution_implemented)
        self.assertFalse(plan.snapshot_selection_mutation_implemented)
    ...
    def test_signals_score_knowledge_rollback_without_execution(self) -> None:
        plan = build_knowledge_rollback(route="generate")
        snapshot_signal_ids = set(plan.knowledge_snapshot_signal_ids)
        ...
        for signal in plan.signals:
            ...
            self.assertEqual(
                ...
                "knowledge_rollback_entry.v1",
            )
            self.assertEqual(signal.route_name, RouteName.GENERATE)
            self.assertEqual(
                signal.signal_id,
                f"knowledge_rollback::{signal.signal_kind}",
            )
            self.assertEqual(
                signal.knowledge_snapshot_signal_count,
            ...
            self.assertEqual(
                ...
                min(
                    ...
                    max(
                        0,
                        signal.rollback_signal_score * 3
                        + signal.snapshot_alignment_score * 3
            ...
            self.assertIn("knowledge_rollback", signal.context_tags)
            self.assertIn(
                "knowledge_rollback_execution",
                signal.blocked_runtime_behaviors,
            )
            self.assertIn("rollback_plan_application", signal.blocked_runtime_behaviors)
            self.assertIn(
                "snapshot_restore_execution",
                signal.blocked_runtime_behaviors,
            )
            self.assertIn("kb_storage_write", signal.blocked_runtime_behaviors)
            self.assertTrue(signal.hitl_required_before_rollback)
            self.assertTrue(signal.knowledge_rollback_capability_implemented)
            self.assertTrue(signal.knowledge_rollback_metadata_implemented)
            self.assertTrue(signal.knowledge_snapshot_metadata_used)
            ...
            self.assertFalse(signal.snapshot_selection_mutation_implemented)
        ...
        inventory = knowledge_rollback_signal_by_id(
        ...
        self.assertEqual(
            len(knowledge_rollback_signals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(knowledge_rollback_signals_for_confidence("high", plan)),
            2,
        )

    def test_plan_rejects_mismatched_knowledge_rollback_metadata(self) -> None:
        plan = build_knowledge_rollback()
        payload = plan.model_dump(mode="json")
        payload["signal_ids"] = ("missing",) + tuple(payload["signal_ids"][1:])

        with self.assertRaisesRegex(ValueError, "signal_ids must match"):
            KnowledgeRollbackPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["overall_rollback_score"] -= 1

        with self.assertRaisesRegex(ValueError, "overall_rollback_score must match"):
            KnowledgeRollbackPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["planned_rollback_ids"] = (plan.signal_ids[0],)

        with self.assertRaisesRegex(
            ValueError,
            "planned_rollback_ids must remain empty",
        ):
            KnowledgeRollbackPlan(**payload)

    def test_knowledge_rollback_composes_task_17_metadata(self) -> None:
        snapshot_plan = build_knowledge_snapshot_engine(route=RouteName.REVIEW)
        plan = build_knowledge_rollback(
            route=RouteName.REVIEW,
            knowledge_snapshot=snapshot_plan,
        )

        self.assertEqual(plan.route_name, RouteName.REVIEW)
        self.assertEqual(plan.knowledge_snapshot_signal_ids, snapshot_plan.signal_ids)
        self.assertEqual(plan.source_count, snapshot_plan.source_count)
        self.assertEqual(plan.domain_count, snapshot_plan.domain_count)
        for signal in plan.signals:
            self.assertTrue(
                set(signal.knowledge_snapshot_signal_ids).issubset(
                    set(snapshot_plan.signal_ids)
                )
            )
            self.assertFalse(signal.knowledge_rollback_execution_implemented)
            self.assertFalse(signal.rollback_plan_application_implemented)
    ...
    def test_knowledge_rollback_preserves_routing_and_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review knowledge rollback posture for p5.js examples.",
    ...
    def test_knowledge_rollback_omits_runtime_mutation_calls(self) -> None:
        plan = build_knowledge_rollback(route=RouteName.GENERATE)
        combined_text = " ".join(
            (
                ...
                *(
                    ...
                    for field in (
                        ...
                        signal.status,
                        signal.confidence,
                        signal.rollback_axis,
                        *signal.knowledge_snapshot_signal_ids,
                        signal.rollback_signal_summary,
                        *signal.context_tags,
