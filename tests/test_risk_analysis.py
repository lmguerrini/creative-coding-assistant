...
from creative_coding_assistant.orchestration import (
    RiskAnalysisPlan,
    build_risk_analysis,
    risk_analysis_proposal_by_id,
    risk_analysis_proposals_for_confidence,
    risk_analysis_proposals_for_status,
    route_request,
)
...
EXPECTED_ROADMAP_ITEMS = ("Risk Analysis",)


class RiskAnalysisTests(unittest.TestCase):
    def test_plan_builds_risk_analysis_metadata(self) -> None:
        plan = build_risk_analysis(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "risk_analysis")
        self.assertEqual(plan.serialization_version, "risk_analysis_plan.v1")
    ...
    def test_proposals_score_risk_without_risk_policy_mutation(
        self,
    ) -> None:
        plan = build_risk_analysis(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "risk_analysis_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "risk_analysis")
            self.assertEqual(proposal.roadmap_item, "Risk Analysis")
            self.assertIn("workflow_risk_engine", proposal.downstream_systems)
            self.assertIn("failure_analysis", proposal.downstream_systems)
            self.assertEqual(
            ...
            self.assertIn(f"risk_score:{proposal.risk_score}", proposal.evidence)
            self.assertIn(
        ...
        signal = risk_analysis_proposal_by_id(
            "risk_analysis::risk_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_risk_analysis_metadata(self) -> None:
        plan = build_risk_analysis()
    ...
    def test_risk_analysis_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review risk analysis proposals.",
