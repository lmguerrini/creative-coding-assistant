...
from creative_coding_assistant.orchestration import (
    ...
    reasoning_evolution_engine_proposal_by_id,
...
EXPECTED_ROADMAP_ITEMS = ("Reasoning Evolution Engine",)


class ReasoningEvolutionEngineTests(unittest.TestCase):
    def test_plan_builds_reasoning_evolution_metadata(self) -> None:
        plan = build_reasoning_evolution_engine(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "reasoning_evolution_engine")
        self.assertEqual(
    ...
    def test_proposals_score_reasoning_without_reasoning_mutation(
        self,
    ) -> None:
        plan = build_reasoning_evolution_engine(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "reasoning_evolution_engine_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "reasoning_evolution_engine")
            self.assertEqual(proposal.roadmap_item, "Reasoning Evolution Engine")
            self.assertIn("creative_reasoning", proposal.downstream_systems)
            self.assertIn("reasoning_budget_optimizer", proposal.downstream_systems)
            self.assertEqual(
        ...
        signal = reasoning_evolution_engine_proposal_by_id(
            "reasoning_evolution_engine::reasoning_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_reasoning_evolution_metadata(self) -> None:
        plan = build_reasoning_evolution_engine()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            ReasoningEvolutionEnginePlan(**payload)
    ...
    def test_reasoning_evolution_does_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review reasoning evolution proposals.",
