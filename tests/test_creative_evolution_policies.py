...
from creative_coding_assistant.orchestration import (
    ...
    creative_evolution_policies_proposal_by_id,
...
EXPECTED_ROADMAP_ITEMS = ("Creative Evolution Policies",)


class CreativeEvolutionPoliciesTests(unittest.TestCase):
    def test_plan_builds_creative_policy_metadata(self) -> None:
        plan = build_creative_evolution_policies(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "creative_evolution_policies")
        self.assertEqual(
    ...
    def test_proposals_score_creative_policies_without_creative_mutation(
        self,
    ) -> None:
        plan = build_creative_evolution_policies(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "creative_evolution_policies_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "creative_evolution_policies")
            self.assertEqual(proposal.roadmap_item, "Creative Evolution Policies")
            self.assertIn("creative_strategy", proposal.downstream_systems)
            self.assertIn("creative_quality_prediction", proposal.downstream_systems)
            self.assertEqual(
        ...
        signal = creative_evolution_policies_proposal_by_id(
            "creative_evolution_policies::creative_policy_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_creative_policy_metadata(self) -> None:
        plan = build_creative_evolution_policies()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            CreativeEvolutionPoliciesPlan(**payload)
    ...
    def test_creative_policies_do_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review creative evolution policy proposals.",
