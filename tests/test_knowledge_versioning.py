...
from creative_coding_assistant.orchestration import (
    ...
    knowledge_versioning_signal_by_id,
    knowledge_versioning_signals_for_confidence,
    knowledge_versioning_signals_for_status,
    route_request,
)
from creative_coding_assistant.orchestration.routing import RouteName

REQUIRED_EXECUTION_MODES = ("manual_mode", "assisted_mode", "auto_mode")
EXPECTED_ROADMAP_ITEMS = ("Knowledge Versioning",)
REQUIRED_SIGNAL_FIELDS = {
    ...
    "execution_mode_id",
    "versioning_axis",
    "knowledge_provenance_signal_ids",
    "knowledge_provenance_signal_count",
    "source_count",
    "domain_count",
    "version_signal_summary",
    "version_signal_score",
    "provenance_alignment_score",
    "lifecycle_alignment_score",
    "mutation_risk_score",
    "governance_weight",
    "versioning_score",
    "hitl_required_before_versioning",
    "context_tags",
    "explainability_notes",
    "advisory_actions",
    "evidence",
    "blocked_runtime_behaviors",
    "knowledge_versioning_capability_implemented",
    "knowledge_versioning_metadata_implemented",
    "knowledge_provenance_metadata_used",
...
class KnowledgeVersioningTests(unittest.TestCase):
    def test_plan_builds_knowledge_versioning_metadata(self) -> None:
        plan = build_knowledge_versioning(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "knowledge_versioning")
        self.assertEqual(plan.serialization_version, "knowledge_versioning_plan.v1")
        self.assertEqual(plan.route_name, RouteName.GENERATE)
        self.assertEqual(plan.task_type, "creative_coding")
        self.assertEqual(
            plan.knowledge_provenance_role,
            "knowledge_provenance_evolution",
        )
        self.assertEqual(
            plan.knowledge_provenance_serialization_version,
            "knowledge_provenance_plan.v1",
        )
        self.assertEqual(len(plan.knowledge_provenance_signal_ids), 5)
        self.assertEqual(plan.knowledge_provenance_signal_count, 5)
        ...
        self.assertFalse(plan.planned_version_ids)
        self.assertFalse(plan.mutated_version_graph_ids)
        self.assertFalse(plan.written_version_record_ids)
        self.assertFalse(plan.assigned_version_ids)
        self.assertFalse(plan.written_version_history_ids)
        self.assertFalse(plan.created_snapshot_ids)
        self.assertFalse(plan.applied_rollback_ids)
        self.assertFalse(plan.written_kb_record_ids)
        self.assertEqual(plan.overall_versioning_posture, "guarded")
        self.assertIn("does not execute knowledge versioning", plan.authority_boundary)
        self.assertIn("write KB storage", plan.authority_boundary)
        self.assertTrue(plan.knowledge_versioning_capability_implemented)
        self.assertTrue(plan.knowledge_versioning_metadata_implemented)
        self.assertTrue(plan.roadmap_item_covered)
        self.assertTrue(plan.knowledge_provenance_metadata_used)
        ...
        self.assertFalse(plan.version_id_assignment_implemented)
        self.assertFalse(plan.version_lineage_reconstruction_implemented)
        self.assertFalse(plan.version_history_write_implemented)
    ...
    def test_signals_score_knowledge_versioning_without_execution(self) -> None:
        plan = build_knowledge_versioning(route="generate")
        provenance_signal_ids = set(plan.knowledge_provenance_signal_ids)
        ...
        for signal in plan.signals:
            ...
            self.assertEqual(
                ...
                "knowledge_versioning_entry.v1",
            )
            self.assertEqual(signal.route_name, RouteName.GENERATE)
            self.assertEqual(
                signal.signal_id,
                f"knowledge_versioning::{signal.signal_kind}",
            )
            self.assertEqual(
                signal.knowledge_provenance_signal_count,
                len(signal.knowledge_provenance_signal_ids),
            )
            self.assertTrue(
                set(signal.knowledge_provenance_signal_ids).issubset(
                    provenance_signal_ids
                )
            )
            self.assertEqual(
                signal.versioning_score,
                min(
                    1000,
                    max(
                        0,
                        signal.version_signal_score * 3
                        + signal.provenance_alignment_score * 3
                        + signal.lifecycle_alignment_score * 2
                        + signal.mutation_risk_score * 2
                        + signal.governance_weight,
                    ),
                ),
            )
            self.assertIn("knowledge_versioning", signal.context_tags)
            self.assertIn(
                "knowledge_versioning_execution",
                signal.blocked_runtime_behaviors,
            )
            self.assertIn("version_graph_mutation", signal.blocked_runtime_behaviors)
            self.assertIn("snapshot_creation", signal.blocked_runtime_behaviors)
            self.assertIn("rollback_execution", signal.blocked_runtime_behaviors)
            self.assertIn("kb_storage_write", signal.blocked_runtime_behaviors)
            self.assertTrue(signal.hitl_required_before_versioning)
            self.assertTrue(signal.knowledge_versioning_capability_implemented)
            self.assertTrue(signal.knowledge_versioning_metadata_implemented)
            self.assertTrue(signal.knowledge_provenance_metadata_used)
        ...
        inventory = knowledge_versioning_signal_by_id(
        ...
        self.assertEqual(
            len(knowledge_versioning_signals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(knowledge_versioning_signals_for_confidence("high", plan)),
            2,
        )

    def test_plan_rejects_mismatched_knowledge_versioning_metadata(self) -> None:
        plan = build_knowledge_versioning()
        payload = plan.model_dump(mode="json")
        payload["signal_ids"] = ("missing",) + tuple(payload["signal_ids"][1:])

        with self.assertRaisesRegex(ValueError, "signal_ids must match"):
            KnowledgeVersioningPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["overall_versioning_score"] -= 1

        with self.assertRaisesRegex(ValueError, "overall_versioning_score must match"):
            KnowledgeVersioningPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["planned_version_ids"] = (plan.signal_ids[0],)

        with self.assertRaisesRegex(
            ValueError,
            "planned_version_ids must remain empty",
        ):
            KnowledgeVersioningPlan(**payload)

    def test_knowledge_versioning_composes_task_15_metadata(self) -> None:
        provenance_plan = build_knowledge_provenance_evolution(route=RouteName.REVIEW)
        plan = build_knowledge_versioning(
            route=RouteName.REVIEW,
            knowledge_provenance=provenance_plan,
        )

        self.assertEqual(plan.route_name, RouteName.REVIEW)
        self.assertEqual(
            plan.knowledge_provenance_signal_ids,
            provenance_plan.signal_ids,
        )
        self.assertEqual(plan.source_count, provenance_plan.source_count)
        self.assertEqual(plan.domain_count, provenance_plan.domain_count)
        for signal in plan.signals:
            self.assertTrue(
                set(signal.knowledge_provenance_signal_ids).issubset(
                    set(provenance_plan.signal_ids)
                )
            )
            self.assertFalse(signal.version_graph_mutation_implemented)
            self.assertFalse(signal.version_record_write_implemented)
    ...
    def test_knowledge_versioning_preserves_routing_and_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review knowledge versioning posture for p5.js examples.",
    ...
    def test_knowledge_versioning_omits_runtime_mutation_calls(self) -> None:
        plan = build_knowledge_versioning(route=RouteName.GENERATE)
        combined_text = " ".join(
            (
                ...
                *(
                    ...
                    for field in (
                        ...
                        signal.status,
                        signal.confidence,
                        signal.versioning_axis,
                        *signal.knowledge_provenance_signal_ids,
                        signal.version_signal_summary,
                        *signal.context_tags,
