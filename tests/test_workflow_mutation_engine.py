...
from creative_coding_assistant.orchestration import (
    ...
    route_request,
    workflow_mutation_engine_proposal_by_id,
    workflow_mutation_engine_proposals_for_confidence,
    workflow_mutation_engine_proposals_for_status,
)
...
EXPECTED_ROADMAP_ITEMS = ("Workflow Mutation Engine",)


class WorkflowMutationEngineTests(unittest.TestCase):
    def test_plan_builds_workflow_mutation_metadata(self) -> None:
        plan = build_workflow_mutation_engine(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "workflow_mutation_engine")
        self.assertEqual(
    ...
    def test_proposals_score_workflow_mutation_without_applying_it(self) -> None:
        plan = build_workflow_mutation_engine(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "workflow_mutation_engine_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "workflow_mutation_engine")
            self.assertEqual(proposal.roadmap_item, "Workflow Mutation Engine")
            self.assertIn("workflow_graph", proposal.downstream_systems)
            self.assertIn("workflow_review", proposal.downstream_systems)
            self.assertEqual(
        ...
        candidate = workflow_mutation_engine_proposal_by_id(
            "workflow_mutation_engine::workflow_mutation_candidate",
            plan,
        )
        ...
        self.assertEqual(
            len(workflow_mutation_engine_proposals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(workflow_mutation_engine_proposals_for_confidence("guarded", plan)),
            2,
        )

    def test_plan_rejects_mismatched_workflow_mutation_metadata(self) -> None:
        plan = build_workflow_mutation_engine()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            WorkflowMutationEnginePlan(**payload)
    ...
    def test_workflow_mutation_engine_does_not_change_routing_or_provider(
        self,
    ) -> None:
        request = AssistantRequest(
            query="Review workflow mutation proposals.",
