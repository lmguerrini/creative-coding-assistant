...
from creative_coding_assistant.orchestration import (
    CostTrendsPlan,
    build_cost_trends,
    cost_trends_proposal_by_id,
    cost_trends_proposals_for_confidence,
    cost_trends_proposals_for_status,
    route_request,
)
...
EXPECTED_ROADMAP_ITEMS = ("Cost Trends",)


class CostTrendsTests(unittest.TestCase):
    def test_plan_builds_cost_trend_proposal_metadata(self) -> None:
        plan = build_cost_trends(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "cost_trends")
        self.assertEqual(plan.serialization_version, "cost_trends_plan.v1")
    ...
    def test_proposals_score_cost_trends_without_budget_mutation(self) -> None:
        plan = build_cost_trends(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "cost_trends_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "cost_trends")
            self.assertEqual(proposal.roadmap_item, "Cost Trends")
            self.assertIn("cost_dashboard", proposal.downstream_systems)
            self.assertIn("budget_policies", proposal.downstream_systems)
            self.assertEqual(
        ...
        pressure = cost_trends_proposal_by_id(
            "cost_trends::cost_pressure_signal",
            plan,
        )
        self.assertIsNotNone(pressure)
        assert pressure is not None
        self.assertEqual(pressure.status, "guarded")
        self.assertEqual(
            len(cost_trends_proposals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(cost_trends_proposals_for_confidence("guarded", plan)),
            2,
        )

    def test_plan_rejects_mismatched_cost_trend_metadata(self) -> None:
        plan = build_cost_trends()
    ...
    def test_cost_trends_do_not_change_routing_or_provider_factory(self) -> None:
        request = AssistantRequest(
            query="Review cost trend evolution suggestions.",
