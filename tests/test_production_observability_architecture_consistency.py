import unittest

from creative_coding_assistant.contracts import (
    AssistantMode,
    AssistantRequest,
    CreativeCodingDomain,
)
from creative_coding_assistant.orchestration import (
    ProductionObservabilityArchitectureRegistry,
    production_observability_architecture_by_surface,
    production_observability_architecture_records_for_layer,
    production_observability_architecture_registry,
    route_request,
)
from creative_coding_assistant.orchestration.routing import RouteName

EXPECTED_SURFACE_IDS = (
    "token_dashboard",
    "cost_dashboard",
    "quality_dashboard",
    "performance_dashboard",
    "production_telemetry",
    "workflow_diagnostics",
    "agent_diagnostics",
    "routing_diagnostics",
    "escalation_diagnostics",
    "failure_analysis",
    "error_intelligence",
    "workflow_health_monitoring",
    "system_health_monitoring",
    "creative_analytics",
    "confidence_analytics",
    "creative_diversity_analytics",
    "runtime_timeline",
    "workflow_explainability_dashboard",
)
EXPECTED_ARCHITECTURE_LAYERS = (
    "dashboard_observability_boundary",
    "telemetry_diagnostics_boundary",
    "failure_health_boundary",
    "creative_confidence_boundary",
    "timeline_explainability_boundary",
)
REQUIRED_RECORD_FIELDS = {
    "surface_id",
    "architecture_layer",
    "architecture_stage",
    "source_role",
    "source_serialization_version",
    "source_count_field",
    "source_count",
    "validated_version_rules",
    "passive_boundary_flags",
    "source_blocked_runtime_behaviors",
    "source_active_runtime_flags",
    "missing_coverage_items",
    "source_observability_only_declared",
    "v5_architecture_consistency_confirmed",
    "v4_boundary_compatibility_confirmed",
    "version_runtime_rules_confirmed",
    "architecture_consistency_status",
    "runtime_metric_collection_implemented",
    "telemetry_emission_implemented",
    "trace_capture_implemented",
    "alert_emission_implemented",
    "workflow_execution_implemented",
    "workflow_control_implemented",
    "workflow_state_mutation_implemented",
    "provider_model_routing_implemented",
    "agent_invocation_implemented",
    "health_check_execution_implemented",
    "live_error_classification_implemented",
    "automated_remediation_implemented",
    "timeline_reconstruction_implemented",
    "explanation_generation_implemented",
    "human_review_request_implemented",
    "retry_triggering_implemented",
    "prompt_mutation_implemented",
    "persistent_storage_write_implemented",
    "generated_output_mutation_implemented",
    "runtime_evolution_implemented",
    "serialization_version",
    "advisory_only",
}


class ProductionObservabilityArchitectureConsistencyTests(unittest.TestCase):
    def test_registry_covers_v5_4_observability_surfaces(self) -> None:
        registry = production_observability_architecture_registry()

        self.assertEqual(
            registry.role,
            "production_observability_architecture_registry",
        )
        self.assertEqual(
            registry.serialization_version,
            "production_observability_architecture_registry.v1",
        )
        self.assertEqual(
            registry.architecture_stage,
            "v5_4_architecture_consistency_pass",
        )
        self.assertEqual(registry.surface_ids, EXPECTED_SURFACE_IDS)
        self.assertEqual(registry.architecture_layers, EXPECTED_ARCHITECTURE_LAYERS)
        self.assertEqual(registry.record_count, 18)
        self.assertTrue(registry.all_surfaces_covered)
        self.assertTrue(registry.no_active_runtime_flags)
        self.assertTrue(registry.no_missing_coverage)
        self.assertTrue(registry.v4_boundaries_preserved)
        self.assertTrue(registry.runtime_evolution_not_applied)
        self.assertIn("runtime_evolution_not_applied", registry.validated_version_rules)
        self.assertIn("telemetry_emission", registry.blocked_runtime_behaviors)
        self.assertIn("does not collect live metrics", registry.authority_boundary)
        self.assertFalse(registry.runtime_metric_collection_implemented)
        self.assertFalse(registry.telemetry_emission_implemented)
        self.assertFalse(registry.trace_capture_implemented)
        self.assertFalse(registry.alert_emission_implemented)
        self.assertFalse(registry.workflow_execution_implemented)
        self.assertFalse(registry.workflow_control_implemented)
        self.assertFalse(registry.workflow_state_mutation_implemented)
        self.assertFalse(registry.provider_model_routing_implemented)
        self.assertFalse(registry.agent_invocation_implemented)
        self.assertFalse(registry.health_check_execution_implemented)
        self.assertFalse(registry.live_error_classification_implemented)
        self.assertFalse(registry.automated_remediation_implemented)
        self.assertFalse(registry.timeline_reconstruction_implemented)
        self.assertFalse(registry.explanation_generation_implemented)
        self.assertFalse(registry.human_review_request_implemented)
        self.assertFalse(registry.retry_triggering_implemented)
        self.assertFalse(registry.prompt_mutation_implemented)
        self.assertFalse(registry.persistent_storage_write_implemented)
        self.assertFalse(registry.generated_output_mutation_implemented)
        self.assertFalse(registry.runtime_evolution_implemented)
        self.assertTrue(registry.advisory_only)

    def test_records_are_passive_and_source_aligned(self) -> None:
        registry = production_observability_architecture_registry()

        for record in registry.records:
            dumped = record.model_dump(mode="json")
            self.assertEqual(set(dumped), REQUIRED_RECORD_FIELDS)
            self.assertEqual(
                record.serialization_version,
                "production_observability_architecture_record.v1",
            )
            self.assertEqual(record.architecture_stage, registry.architecture_stage)
            self.assertTrue(record.source_serialization_version.endswith(".v1"))
            self.assertIn(
                record.source_count_field,
                {
                    "panel_count",
                    "channel_count",
                    "record_count",
                    "surface_count",
                    "diagnostic_signal_count",
                    "agent_signal_count",
                    "routing_signal_count",
                    "escalation_signal_count",
                    "failure_signal_count",
                    "error_signal_count",
                    "health_signal_count",
                    "system_signal_count",
                    "creative_signal_count",
                    "confidence_signal_count",
                    "diversity_signal_count",
                    "timeline_signal_count",
                    "explainability_signal_count",
                    "telemetry_signal_count",
                },
            )
            self.assertGreaterEqual(record.source_count, 1)
            self.assertEqual(
                record.validated_version_rules,
                registry.validated_version_rules,
            )
            self.assertEqual(
                record.passive_boundary_flags,
                registry.passive_boundary_flags,
            )
            self.assertTrue(record.source_blocked_runtime_behaviors)
            self.assertFalse(record.source_active_runtime_flags)
            self.assertFalse(record.missing_coverage_items)
            self.assertTrue(record.source_observability_only_declared)
            self.assertTrue(record.v5_architecture_consistency_confirmed)
            self.assertTrue(record.v4_boundary_compatibility_confirmed)
            self.assertTrue(record.version_runtime_rules_confirmed)
            self.assertEqual(record.architecture_consistency_status, "pass")
            self.assertFalse(record.runtime_metric_collection_implemented)
            self.assertFalse(record.telemetry_emission_implemented)
            self.assertFalse(record.trace_capture_implemented)
            self.assertFalse(record.alert_emission_implemented)
            self.assertFalse(record.workflow_execution_implemented)
            self.assertFalse(record.workflow_control_implemented)
            self.assertFalse(record.workflow_state_mutation_implemented)
            self.assertFalse(record.provider_model_routing_implemented)
            self.assertFalse(record.agent_invocation_implemented)
            self.assertFalse(record.health_check_execution_implemented)
            self.assertFalse(record.live_error_classification_implemented)
            self.assertFalse(record.automated_remediation_implemented)
            self.assertFalse(record.timeline_reconstruction_implemented)
            self.assertFalse(record.explanation_generation_implemented)
            self.assertFalse(record.human_review_request_implemented)
            self.assertFalse(record.retry_triggering_implemented)
            self.assertFalse(record.prompt_mutation_implemented)
            self.assertFalse(record.persistent_storage_write_implemented)
            self.assertFalse(record.generated_output_mutation_implemented)
            self.assertFalse(record.runtime_evolution_implemented)
            self.assertTrue(record.advisory_only)

    def test_lookup_and_layer_filters_are_stable(self) -> None:
        registry = production_observability_architecture_registry()
        explainability = production_observability_architecture_by_surface(
            "workflow_explainability_dashboard",
            registry,
        )
        missing = production_observability_architecture_by_surface(
            "missing",
            registry,
        )
        dashboards = production_observability_architecture_records_for_layer(
            "dashboard_observability_boundary",
            registry,
        )
        diagnostics = production_observability_architecture_records_for_layer(
            "telemetry_diagnostics_boundary",
            registry,
        )
        timeline = production_observability_architecture_records_for_layer(
            "timeline_explainability_boundary",
            registry,
        )

        self.assertIsNone(missing)
        self.assertIsNotNone(explainability)
        assert explainability is not None
        self.assertEqual(
            explainability.source_role,
            "workflow_explainability_dashboard",
        )
        self.assertEqual(explainability.source_count_field, "panel_count")
        self.assertEqual(
            tuple(record.surface_id for record in dashboards),
            (
                "token_dashboard",
                "cost_dashboard",
                "quality_dashboard",
                "performance_dashboard",
            ),
        )
        self.assertEqual(
            tuple(record.surface_id for record in diagnostics),
            (
                "production_telemetry",
                "workflow_diagnostics",
                "agent_diagnostics",
                "routing_diagnostics",
                "escalation_diagnostics",
            ),
        )
        self.assertEqual(
            tuple(record.surface_id for record in timeline),
            ("runtime_timeline", "workflow_explainability_dashboard"),
        )

    def test_registry_rejects_mismatched_or_active_records(self) -> None:
        registry = production_observability_architecture_registry()
        first_record = registry.records[0]
        duplicate_record = first_record.model_copy(
            update={"architecture_layer": "failure_health_boundary"}
        )
        missing_record = first_record.model_copy(
            update={"missing_coverage_items": ("serialization_version_missing",)}
        )
        active_record = first_record.model_copy(
            update={"source_active_runtime_flags": ("workflow_execution_implemented",)}
        )

        with self.assertRaisesRegex(ValueError, "surface_ids must be unique"):
            self._registry_with_records(
                (first_record, duplicate_record) + registry.records[2:]
            )

        with self.assertRaisesRegex(ValueError, "missing coverage"):
            self._registry_with_records((missing_record,) + registry.records[1:])

        with self.assertRaisesRegex(ValueError, "active runtime flags"):
            self._registry_with_records((active_record,) + registry.records[1:])

    def test_architecture_pass_does_not_change_request_routing(self) -> None:
        request = AssistantRequest(
            query="Generate production observability architecture metadata.",
            mode=AssistantMode.GENERATE,
            domains=(CreativeCodingDomain.P5_JS,),
        )
        baseline_decision = route_request(request)

        production_observability_architecture_registry()
        production_observability_architecture_by_surface("runtime_timeline")
        production_observability_architecture_records_for_layer(
            "failure_health_boundary"
        )
        next_decision = route_request(request)

        self.assertEqual(next_decision, baseline_decision)
        self.assertEqual(next_decision.route, RouteName.GENERATE)

    def test_architecture_metadata_does_not_declare_active_terms(self) -> None:
        registry = production_observability_architecture_registry()
        combined_text = " ".join(
            (
                registry.authority_boundary,
                *registry.blocked_runtime_behaviors,
                *registry.passive_boundary_flags,
                *(
                    field
                    for record in registry.records
                    for field in (
                        record.surface_id,
                        record.architecture_layer,
                        *record.validated_version_rules,
                        *record.passive_boundary_flags,
                    )
                ),
            )
        )

        for forbidden_term in (
            "collect_metrics(",
            "emit_telemetry(",
            "capture_trace(",
            "emit_alert(",
            "execute_workflow(",
            "control_workflow(",
            "route_provider(",
            "invoke_agent(",
            "classify_live_error(",
            "remediate_failure(",
            "reconstruct_timeline(",
            "generate_explanation(",
            "request_human_review(",
            "trigger_retry(",
            "mutate_prompt(",
            "write_storage(",
            "modify_output(",
            "apply_runtime_evolution(",
        ):
            self.assertNotIn(forbidden_term, combined_text)

    def _registry_with_records(
        self,
        records: tuple,
    ) -> ProductionObservabilityArchitectureRegistry:
        registry = production_observability_architecture_registry()
        return ProductionObservabilityArchitectureRegistry(
            records=records,
            surface_ids=tuple(record.surface_id for record in records),
            record_count=len(records),
            architecture_layers=registry.architecture_layers,
            validated_version_rules=registry.validated_version_rules,
            passive_boundary_flags=registry.passive_boundary_flags,
        )


if __name__ == "__main__":
    unittest.main()
