...
from creative_coding_assistant.orchestration import (
    ...
    route_request,
    workflow_evolution_proposal_by_id,
    workflow_evolution_proposals_for_confidence,
    workflow_evolution_proposals_for_status,
)
...
EXPECTED_ROADMAP_ITEMS = ("Workflow Evolution",)
...
class WorkflowEvolutionTests(unittest.TestCase):
    def test_plan_builds_workflow_evolution_proposal_metadata(self) -> None:
        plan = build_workflow_evolution(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "workflow_evolution")
        self.assertEqual(plan.serialization_version, "workflow_evolution_plan.v1")
    ...
    def test_proposals_score_and_explain_without_workflow_mutation(self) -> None:
        plan = build_workflow_evolution(route="generate")
        source_ids = set(plan.upstream_signal_source_ids)

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "workflow_evolution_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "workflow_evolution")
            self.assertEqual(proposal.roadmap_item, "Workflow Evolution")
            ...
            self.assertIn("workflow_graph", proposal.downstream_systems)
            self.assertEqual(
            ...
            self.assertIn("V6.4 research", proposal.upstream_signal_explanation)
            self.assertIn("workflow_mutation", proposal.blocked_runtime_behaviors)
            self.assertIn("routing_mutation", proposal.blocked_runtime_behaviors)
        ...
        workflow_gap = workflow_evolution_proposal_by_id(
            "workflow_evolution::workflow_success_pattern_gap",
            plan,
        )
        self.assertIsNotNone(workflow_gap)
        assert workflow_gap is not None
        self.assertEqual(workflow_gap.status, "guarded")
        self.assertEqual(
            len(workflow_evolution_proposals_for_status("review_required", plan)),
            3,
        )
        self.assertEqual(
            len(workflow_evolution_proposals_for_confidence("guarded", plan)),
            2,
        )

    def test_plan_rejects_mismatched_workflow_evolution_metadata(self) -> None:
        plan = build_workflow_evolution()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            WorkflowEvolutionPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["mutated_workflow_ids"] = (plan.proposal_ids[0],)
    ...
    def test_workflow_evolution_does_not_change_routing_or_provider_factory(
        self,
    ) -> None:
        request = AssistantRequest(
            query="Review workflow evolution suggestions.",
