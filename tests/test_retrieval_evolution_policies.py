...
EXPECTED_ROADMAP_ITEMS = ("Retrieval Evolution Policies",)


class RetrievalEvolutionPoliciesTests(unittest.TestCase):
    def test_plan_builds_retrieval_policy_metadata(self) -> None:
        plan = build_retrieval_evolution_policies(route=RouteName.GENERATE)

        self.assertEqual(plan.role, "retrieval_evolution_policies")
        self.assertEqual(
    ...
    def test_proposals_score_retrieval_policies_without_retrieval_mutation(
        self,
    ) -> None:
        plan = build_retrieval_evolution_policies(route="generate")

        for proposal in plan.proposals:
            self.assertEqual(
                proposal.serialization_version,
                "retrieval_evolution_policies_proposal.v1",
            )
            self.assertEqual(proposal.surface_role, "retrieval_evolution_policies")
            self.assertEqual(proposal.roadmap_item, "Retrieval Evolution Policies")
            self.assertIn("retrieval_foundation", proposal.downstream_systems)
            self.assertIn(
        ...
        signal = retrieval_evolution_policies_proposal_by_id(
            "retrieval_evolution_policies::retrieval_policy_signal_alignment",
            plan,
        )
    ...
    def test_plan_rejects_mismatched_retrieval_policy_metadata(self) -> None:
        plan = build_retrieval_evolution_policies()
        ...
        with self.assertRaisesRegex(ValueError, "proposal_ids must match"):
            RetrievalEvolutionPoliciesPlan(**payload)

        payload = plan.model_dump(mode="json")
        payload["mutated_retrieval_ids"] = (plan.proposal_ids[0],)
    ...
    def test_retrieval_policies_do_not_change_routing_or_provider(self) -> None:
        request = AssistantRequest(
            query="Review retrieval evolution policy proposals.",
