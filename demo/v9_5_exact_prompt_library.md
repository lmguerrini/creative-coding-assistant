...
```text
Create exactly one self-contained .three.js artifact named kinetic-orbit-sculpture.three.js. Use plain JavaScript only: THREE.Scene, PerspectiveCamera, WebGLRenderer, a torus or sphere sculpture, two lights, a dark background, and an animation loop with slow camera orbit. No imports, HTML, React, TypeScript, Markdown, or prose. Return only the artifact.
```
