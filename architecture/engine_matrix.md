...
This document catalogs cross-cutting architecture layers, metadata, and
contract boundaries recorded across the repository's version history. Versions are chronological delivery increments.
Here they identify when a registry or contract was introduced;
they are not a forward delivery schedule. Engines are catalog groupings, not a
...
> provider path, persistence boundaries, and browser preview lane in those
> documents take precedence over historical wording in registry names.
...
- [symbolic_translation_engine.md](symbolic_translation_engine.md) for the
  bounded symbolic translation contract and runtime boundary
- [sacred_geometry_engine.md](sacred_geometry_engine.md) for the bounded sacred
  geometry and mathematics contract and runtime boundary
- [sacred_architecture_engine.md](sacred_architecture_engine.md) for the bounded
  sacred architecture and reverse-engineering contract and runtime boundary
- [mythopoetic_narrative_engine.md](mythopoetic_narrative_engine.md) for the
  bounded mythopoetic narrative contract and runtime boundary
- [immersive_audiovisual_composer.md](immersive_audiovisual_composer.md) for
  the bounded immersive composition contract and preview boundary
- [hologenesis_core.md](hologenesis_core.md) for the bounded HoloGenesis
  report/planning layer and external-integration boundary

## Engine Layers

| Engine | Purpose | Cataloged boundary | Key examples |
| --- | --- | --- | --- |
...
| Knowledge Engine | Owns retrieval, source grounding, memory, and passive knowledge-reasoning interfaces. | Active and implemented for retrieval and memory; V6.2 adds advisory creative memory metadata, V6.3 adds advisory knowledge evolution metadata, V6.4 adds advisory research metadata, V6.5 adds advisory cross-capability self-evolution signal reading, and V6.6 composes learning, memory, knowledge, research, self-evolution, and cognitive-core metadata through the Unified Cognitive Graph without changing retrieval, KB storage, memory storage, external source access, or long-horizon adaptation behavior; HoloMind remains outside the executable runtime. | Source registry, KB retrieval, prompt memory, grounded prompt input |
| Execution Engine | Owns workflow orchestration, provider execution, validation, artifact extraction, preview preparation, metadata serialization, backend dev mounting, and advisory optimization metadata. | Active and implemented for the bounded LangGraph runtime; V3.6 stabilizes graph assembly and serialization seams, V5 catalogs Execution Optimization & Production Intelligence metadata, and V6 catalogs adaptive learning, creative memory, knowledge evolution, autonomous research, self-evolution governance, and Cognitive OS execution-graph posture metadata without changing runtime control. | Workflow graph, generation, review gate, refinement loop, artifact extraction, preview preparation, workflow metadata payloads, backend bridge |
...
| Creative knowledge distillation | Typed creative knowledge records preserve source provenance, confidence, taxonomy, and repository relationships | Read-only records distinguish registered source coverage from indexed Chroma content without fetching sources, writing storage, or mutating retrieval configuration | The records do not route providers, execute workflows, alter prompts, or mutate generated output | Product surfaces consume this metadata only when explicitly wired |
| Symbolic translation | Bounded contracts map symbolic, artistic, geometric, ritual, aesthetic, and conceptual intent into operational creative-coding guidance without authoritative interpretation | Existing creative knowledge, translation, semantic-motif, and narrative metadata are composed instead of duplicated | The contracts do not mutate LangGraph, provider routing, prompts, storage, or preview runtime | Reports remain passive metadata unless a product surface consumes them |
| Sacred geometry | Bounded ratios, polygons, radial systems, tessellations, recursion, morphogenesis, fields, and motion/light/audio mappings avoid metaphysical or tradition-authority claims | Provenance, symbolic motifs, and a deterministic geometry catalog remain inspectable inputs | The contracts do not mutate preview runtime, route providers, write storage, control workflows, generate assets, or execute external tools | Reports remain passive metadata unless a product surface consumes them |
| Sacred architecture | Spatial layout, topology, proportion, threshold, procession, installation, and textual reverse-engineering guidance excludes real-building analysis claims | Provenance, symbolic-to-spatial motifs, and geometry-to-architecture signals compose architecture-specific reports | The contracts do not perform reconstruction, CAD/BIM/DCC integration, safety certification, preview mutation, routing, storage writes, or workflow control | Reports remain passive metadata unless a product surface consumes them |
| Mythopoetic narrative | Bounded archetypal transformation, journey arcs, procession, emotional arcs, scene sequences, creative briefs, and project storytelling avoid authoritative religious, esoteric, or psychological claims | Symbolic, geometry, and spatial guidance compose narrative-specific reports | The contracts do not mutate preview runtime, route providers, write storage, control workflows, generate audiovisual graphs, or execute external tools | Reports remain passive metadata unless a product surface consumes them |
| Immersive audiovisual composition | Scene graphs, visual language, lighting, color, geometry animation, particles, spatial audio, timing, transitions, installation flow, dramaturgy, and emotional resonance remain explainable creative guidance | Knowledge, symbolic, geometry, spatial, narrative, audio-visual, and style metadata compose one bounded report | The contracts do not mutate previews, add frontend execution, route providers, write storage, control workflows, export previews, or execute external tools | Reports remain passive metadata unless a product surface consumes them |
| HoloGenesis planning | A bounded report layer composes unified graphs, blackboard entries, schedules, creative plans, artistic decisions, curatorial reasoning, readiness, and project-bundle outlines | Existing symbolic, knowledge, geometry, narrative, and installation projections are composed rather than duplicated | The layer does not execute external tools, route providers, mutate workflows, write storage, or mutate previews | Reports remain passive metadata unless a product surface consumes them |
| Demo asset metadata | Scenario prompts, fallback plans, and evidence references make local demo readiness inspectable | Existing retrieval benchmark and evaluation documentation remain the source boundary | Metadata does not execute providers, retrieval, previews, routing, workflows, artifact mutation, storage writes, or external tools | Demo Mode uses normal product paths and labels static current-product references as non-live evidence |
...
metadata descriptions, not dynamic engine routers or executable behavior.
...
extension-hook fields descriptive only: they do not invoke agents, route providers,
select runtimes, execute artifacts, repair previews, trigger retries, or change
generated output.

## V3.6 Audit And Future-Readiness Metadata (Legacy Catalog Label)

V3.6 also exposes passive metadata registries for architecture audit,
consistency checks, and legacy readiness-handoff metadata. These registries are explicit
...
| Registry | Source module | Count | Serialization version | Current boundary |
| --- | --- | ---: | --- | --- |
| Agent capability readiness | `src/creative_coding_assistant/orchestration/agent_capabilities.py` | 6 | `agent_capability_registry.v1` | Describes passive agent capability readiness metadata without creating agents or changing workflow control |
...
| Hybrid agentic workflow readiness | `src/creative_coding_assistant/orchestration/hybrid_agentic_workflow.py` | 5 | `hybrid_workflow_registry.v1` | Maps existing V3 workflow nodes to passive readiness stages without mutating the graph |
...
surfaces for inspection and optional orchestration references; they are not
active multi-agent orchestration.

| Registry | Source module | Count | Serialization version | Current boundary |
| --- | --- | ---: | --- | --- |
| Agent contracts | `src/creative_coding_assistant/orchestration/agent_contracts.py` | 12 | `agent_contract_registry.v1` | Describes per-agent passive inputs, outputs, capabilities, memory posture, cost, latency, and orchestration hook fields |
...
| Agent memory contracts | `src/creative_coding_assistant/orchestration/agent_memory_contracts.py` | 12 | `agent_memory_contract_registry.v1` | Describes session, artifact, evaluation, provenance, and passive blackboard read/write/reference boundaries without storage |
...
| Agent metadata | `src/creative_coding_assistant/orchestration/agent_metadata.py` | 12 | `agent_metadata_registry.v1` | Provides advisory cacheability, parallelization, observability, auditability, cost, latency, and readiness metadata |
...
relationships for passive Agentic Studio references. They are not active
...
| Parallel scheduling | `src/creative_coding_assistant/orchestration/agent_parallel_scheduling.py` | 6 groups | `parallel_scheduling_registry.v1` | Describes candidate concurrency groups without parallel execution, agent invocation, or workflow control |
...
the V4.1/V4.2 agent contract layers. The registries describe candidate escalation
...
| Specialist agent loops | `src/creative_coding_assistant/orchestration/hybrid_agentic_workflow.py` | 5 loops | `specialist_agent_loop_registry.v1` | Describes bounded loop candidates without executing loops or coordinating agents |
...
| Decision provenance | `src/creative_coding_assistant/orchestration/hybrid_agentic_workflow.py` | 4 profiles | `decision_provenance_registry.v1` | Describes candidate decision lineage without recording provenance or writing memory |
| Escalation trace | `src/creative_coding_assistant/orchestration/hybrid_agentic_workflow.py` | 4 profiles | `escalation_trace_registry.v1` | Describes candidate trace context without capturing or emitting traces |
...
| Result normalization | `src/creative_coding_assistant/orchestration/hybrid_agentic_workflow.py` | 4 profiles | `result_normalization_registry.v1` | Describes candidate result packet surfaces without transforming or rewriting outputs |
...
The registries describe passive local/cloud model inspection, Studio mode
...
surface. The registries describe passive preview inspection, multi-preview
...
verification, repository review classification, and bounded
...
traceability, coverage verification, repository review classification,
and bounded capability-scoped fixes. These surfaces are typed contracts and
...
metadata, exposed signals, missing-metadata behavior, and named passive hooks
...
| `session_intelligence` | Session readiness, completion, warnings, and operator next actions | Advisory context for passive agentic-studio handoff metadata |
| `workflow_explorer` | Workflow nodes, edges, active step, runtime status, and progress | Workflow context for passive agentic review without changing graph control |
| `provenance_engine` | Evidence, dependency, artifact, evaluation, final payload, and missing-source provenance | Lineage context for passive creative-evolution metadata without external fetching |
| `creative_timeline` | Ordered request, planning, retrieval, creative, artifact, evaluation, and final stages | Timeline context for passive lineage and learning signals |
| `v3_inspector_panels` | Creative intelligence, generative design, artifact intelligence, evaluation, and provenance records | Review context for passive agentic and adaptive-execution metadata |
| `workstation_dashboard` | Quality, confidence, consistency, readiness, runtime fit, evaluation, workflow health, and HITL cards | Operator policy signal for passive adaptive-execution metadata without autonomous action |
...
metadata boundary that registry readers can consume without making the
workstation responsible for that behavior.
...
- V5.6 is the production-release readiness metadata layer over the stable
  ...
  Evolution, or mutating output. V6 HoloGenesis Core OS entries remain passive
  catalog surfaces, not deferred runtime work.
